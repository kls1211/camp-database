...
async function loadDefaultData(){
  try{
    const res = await fetch("data/japan_campsites.json");
    DATA = await res.json();
  }catch(e){
    DATA = [];
  }
...
async function onCsvImport(evt){
  ...
  try{
    ...
    for (const r of required){ if (idx(r) === -1) throw new Error("CSVヘッダに " + r + " が必要です。"); }

    // optional monthly columns m1..m12
    ...
    const arr = rows.map(line => {
      ...
      if (hasMonthly){
        ...
        for (let k=1;k<=12;k++){
          const id = idx("m"+k);
          if (id !== -1 && cols[id] !== "") monthly[String(k)] = Number(cols[id]);
        }
      }
      return {
        name: cols[idx("name")],
